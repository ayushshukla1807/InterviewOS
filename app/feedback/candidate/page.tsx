...
function StatusBadge({ value, thresholds = [70, 50] }: { value: number; thresholds?: [number, number] }) {
  const status = value >= thresholds[0] ? { label: 'Strong', color: 'text-zinc-100 bg-zinc-800/50 border-zinc-700' }
  ...
  return <span className={`px-2.5 py-1 rounded-full text-[8px] font-medium  tracking-tight border ${status.color}`}>{status.label}</span>;
}
    ...
    <div data-theme="dark" className="min-h-screen bg-[#0A0A0A] text-zinc-300 font-sans">
      ...
      <header className="sticky top-0 z-50 px-8 py-4 border-b border-zinc-800/50 bg-[#0A0A0A]/90 backdrop-blur-xl flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-medium text-white  tracking-tight">Your Interview Report</p>
            <p className="text-[9px] font-bold text-zinc-500  tracking-tight">{name}</p>
          </div>
        </div>
      ...
      <div className="max-w-3xl mx-auto p-8 space-y-8 print:p-0 print:max-w-none">
        ...
        {report.simulation && (
          <section className="space-y-4 print:my-0">
            <h2 className="text-[10px] font-medium text-zinc-500  tracking-[0.3em] print:hidden">Credentials & Certification</h2>
            <div className="relative bg-[#0e0e12] border-2 border-amber-500/20 rounded-[32px] p-8 md:p-12 overflow-hidden shadow-2xl print:border-slate-800 print:bg-white print:text-black">
              ...
              <div className="absolute top-4 left-4 right-4 bottom-4 border border-zinc-800/50 rounded-[24px] pointer-events-none print:border-slate-300" />
              
              <div className="relative space-y-8 text-center">
                {/* Gold Seal Header */}
                <div className="flex flex-col items-center justify-center gap-3">
                  <div className="w-16 h-16 rounded-full bg-gradient-to-br from-amber-400 to-yellow-600 p-0.5 shadow-sm print:shadow-none print:from-amber-500 print:to-amber-600">
                  ...
                  <p className="text-[8px] font-medium text-amber-400  tracking-[0.4em] print:text-amber-600">Verified Job Simulation Accomplishment</p>
                </div>

                <div className="space-y-2">
                  <h3 className="text-2xl md:text-3xl font-medium text-white tracking-tight font-serif  print:text-black">Certificate of Completion</h3>
                  <p className="text-zinc-500 text-xs font-medium print:text-zinc-600">This credential validates that the candidate successfully completed a structured industry work simulation.</p>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-6 max-w-lg mx-auto pt-4 text-left print:text-slate-700">
                  <div>
                    <p className="text-[8px] font-medium text-zinc-500  tracking-tight print:text-zinc-600">Verification Hash</p>
                    <p className="text-[9px] font-mono text-zinc-400 mt-1  print:text-slate-700">OS-{Math.abs(name.split('').reduce((a,b)=>(((a<<5)-a)+b.charCodeAt(0))|0,0)).toString(16).toUpperCase()}-{report.simulation.id?.toUpperCase()}</p>
                  </div>
                  <div className="sm:text-right">
                    <p className="text-[8px] font-medium text-zinc-500  tracking-tight print:text-zinc-600">Verification Status</p>
                    <p className="text-[9px] font-medium text-zinc-100  tracking-tight mt-1 print:text-white">100% Verified Accomplishment</p>
                  </div>
                </div>
                ...
                <div className="pt-6 flex justify-center print:hidden">
                  <button 
                    ...
                    className="flex items-center gap-2 px-6 py-3 bg-white text-black hover:bg-slate-200 text-[10px] font-medium  tracking-tight rounded-xl transition-all shadow-[0_4px_20px_rgba(255,255,255,0.1)]"
                  >
        ...
        <section className="space-y-4 print:hidden">
          <h2 className="text-[10px] font-medium text-zinc-500  tracking-[0.3em]">1. Performance Snapshot</h2>
          <div className="bg-[#111111] border border-zinc-800/50 rounded-2xl overflow-hidden">
            {areas.map((a, i) => (
              <div key={i} className={`px-6 py-4 flex items-center justify-between gap-4 ${i !== areas.length - 1 ? 'border-b border-zinc-800/50' : ''}`}>
                <p className="text-xs font-bold text-zinc-400 w-40 shrink-0">{a.label}</p>
                <div className="flex-1 h-1.5 bg-zinc-900/50 rounded-full overflow-hidden">
                ...
                <div className="flex items-center gap-3 w-32 justify-end">
                  <span className="text-[11px] font-medium text-white tabular-nums">{a.value}/100</span>
        ...
        {perception && (
          <section className="space-y-4">
            <h2 className="text-[10px] font-medium text-zinc-500  tracking-[0.3em]">2. Recruiter Likely Perception</h2>
            <div className="bg-amber-500/5 border border-amber-500/15 rounded-2xl p-6 space-y-4">
              <p className="text-[9px] font-medium text-amber-500  tracking-tight">What the interviewer probably thinks</p>
              <p className="text-sm font-medium text-zinc-400 leading-relaxed">{perception}</p>
            </div>
          </section>
        )}
        ...
        {/* ── WHAT HELPED / HURTING ── */}
        <section className="space-y-4">
          <h2 className="text-[10px] font-medium text-zinc-500  tracking-[0.3em]">3 & 4. Signal Analysis</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-zinc-900/50 border border-emerald-500/15 rounded-2xl p-6 space-y-4">
              <p className="text-[9px] font-medium text-white  tracking-tight">What Helped You</p>
              <ul className="space-y-3">
                {whatHelped.map((s: string, i: number) => (
                  <li key={i} className="flex items-start gap-2"><CheckCircle className="w-3.5 h-3.5 text-white shrink-0 mt-0.5" /><p className="text-xs font-medium text-zinc-400 leading-relaxed">{s}</p></li>
                ))}
              </ul>
            </div>
            <div className="bg-rose-500/5 border border-rose-500/15 rounded-2xl p-6 space-y-4">
              <p className="text-[9px] font-medium text-rose-500  tracking-tight">What Is Hurting You</p>
              <ul className="space-y-3">
                {whatHurting.map((s: string, i: number) => (
                  <li key={i} className="flex items-start gap-2"><AlertCircle className="w-3.5 h-3.5 text-rose-500 shrink-0 mt-0.5" /><p className="text-xs font-medium text-zinc-400 leading-relaxed">{s}</p></li>
                ))}
              </ul>
            </div>
          </div>
        ...
        {mode === 'deepdive' && whyRejected.length > 0 && (
          <section className="space-y-4">
            <h2 className="text-[10px] font-medium text-zinc-500  tracking-[0.3em]">5. Why You May Be Getting Rejected</h2>
            <div className="bg-[#111111] border border-rose-500/10 rounded-2xl p-6 space-y-3">
              {whyRejected.map((r: string, i: number) => (
                <div key={i} className="flex items-start gap-3"><XCircle className="w-3.5 h-3.5 text-rose-500 shrink-0 mt-0.5" /><p className="text-xs font-medium text-zinc-400 leading-relaxed">{r}</p></div>
              ))}
            </div>
          </section>
        )}
        ...
        {/* ── IMPROVEMENT PLAN ── */}
        <section className="space-y-4">
          <h2 className="text-[10px] font-medium text-zinc-500  tracking-[0.3em]">6. Fast Improvement Plan</h2>
        ...
        {/* ── FINAL VERDICT ── */}
        <section className="space-y-4 print:hidden">
          <h2 className="text-[10px] font-medium text-zinc-500  tracking-[0.3em]">8. Final Verdict</h2>
          <div className="bg-[#111111] border border-zinc-800/50 rounded-2xl p-6 space-y-4">
            {ev.finalVerdict && <p className="text-sm font-medium text-zinc-400 leading-relaxed italic">"{ev.finalVerdict}"</p>}
            <div className="flex flex-wrap gap-3 pt-2 border-t border-zinc-800/50">
              <div>
                <p className="text-[8px] font-medium text-zinc-500  tracking-tight mb-1">Overall</p>
                <p className="text-2xl font-medium text-white">{overall}/100</p>
              </div>
              <div className="ml-auto flex flex-col items-end justify-end gap-1">
                <p className={`text-sm font-medium  tracking-tight ${rec === 'Hire' ? 'text-zinc-100' : rec === 'Proceed with Caution' ? 'text-amber-400' : 'text-rose-400'}`}>{rec}</p>
                <p className="text-[9px] font-bold text-zinc-500  tracking-tight">Recruiter Decision</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <div className="flex items-center justify-between pt-8 border-t border-zinc-800/50">
          ...
          <Link href={`/feedback/recruiter?name=${encodeURIComponent(name)}`}
            className="px-6 py-2.5 bg-white text-black rounded-xl text-[10px] font-medium  tracking-tight hover:bg-slate-200 transition-all">
...
export default function CandidateReportPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#0A0A0A] flex items-center justify-center text-zinc-500 text-[10px] font-medium  tracking-tight ">Loading...</div>}>
      <CandidateContent />
    </Suspense>
  );
}

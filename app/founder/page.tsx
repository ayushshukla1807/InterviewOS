        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-bold text-lg tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-white to-white/70">
              InterviewOS <span className="text-indigo-400 font-mono text-sm ml-2 px-2 py-0.5 bg-indigo-500/10 rounded-full border border-indigo-500/20">FOUNDER</span>
            </span>
          </div>
      ...
      <main className="pt-24 pb-12 px-6 max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div>
            ...
            <p className="text-slate-400 text-sm">Global overview of all platform activity, users, and neural sessions.</p>
          </div>
          
          {/* Quick Stats row */}
          <div className="flex gap-4">
            <div className="px-5 py-3 rounded-xl bg-white/[0.02] border border-white/5 backdrop-blur-sm">
              <div className="text-xs text-slate-500 mb-1 font-medium">Total Users</div>
              <div className="text-2xl font-bold text-white font-mono">{dbUsers.length}</div>
            </div>
            <div className="px-5 py-3 rounded-xl bg-white/[0.02] border border-white/5 backdrop-blur-sm">
              <div className="text-xs text-slate-500 mb-1 font-medium">Completed Sims</div>
              <div className="text-2xl font-bold text-white font-mono">{dbReports.length}</div>
            </div>
            <div className="px-5 py-3 rounded-xl bg-white/[0.02] border border-white/5 backdrop-blur-sm">
              <div className="text-xs text-indigo-400/80 mb-1 font-medium">System Health</div>
        ...
        {/* Tab Navigation */}
        <div className="flex space-x-1 border-b border-white/10 mb-8 overflow-x-auto no-scrollbar">
          {[
        ...
        <AnimatePresence mode="wait">
          {isLoadingDb ? (
            <motion.div 
              ...
              className="py-20 flex flex-col items-center justify-center text-slate-500 gap-4"
            >
              <div className="w-8 h-8 border-2 border-indigo-500/30 border-t-indigo-500 rounded-full animate-spin" />
              <div className="text-xs font-mono tracking-widest uppercase">Querying Database...</div>
            </motion.div>
          ) : (
            <motion.div
              key={activeTab}
            ...
            >
              ...
              {activeTab === 'overview' && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="col-span-1 md:col-span-2 p-6 rounded-2xl bg-white/[0.02] border border-white/5 backdrop-blur-md">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-4 rounded-xl bg-black/20 border border-white/5">
                        <div className="text-xs text-slate-500 mb-1">Global Avg Score</div>
                      ...
                      <div className="p-4 rounded-xl bg-black/20 border border-white/5">
                        <div className="text-xs text-slate-500 mb-1">Total Recruiters</div>
                        <div className="text-3xl font-mono text-white">{totalRecruiters}</div>
                      </div>
                      <div className="p-4 rounded-xl bg-black/20 border border-white/5">
                        <div className="text-xs text-slate-500 mb-1">Database Uptime</div>
                        <div className="text-3xl font-mono text-emerald-400">99.9%</div>
                      </div>
                    </div>
                  </div>

                  <div className="col-span-1 p-6 rounded-2xl bg-white/[0.02] border border-white/5 backdrop-blur-md flex flex-col">
                    <h3 className="text-sm font-semibold text-white mb-6 flex items-center gap-2">
                      <Zap className="w-4 h-4 text-indigo-400" />
                      Quick Actions
                    </h3>
                    <div className="space-y-3 flex-1">
                      <Link href="/recruiter" className="w-full flex items-center justify-between p-3 rounded-lg bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 hover:bg-indigo-500/20 transition-colors">
                        <span className="text-sm font-medium">Recruiter View</span>
                        <ArrowRight className="w-4 h-4" />
                      </Link>
                      <Link href="/candidate" className="w-full flex items-center justify-between p-3 rounded-lg bg-white/5 border border-white/5 text-slate-300 hover:bg-white/10 transition-colors">
              ...
              {activeTab === 'users' && (
                <div className="p-6 rounded-2xl bg-white/[0.02] border border-white/5 backdrop-blur-md">
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="text-xs text-slate-500">{dbUsers.length} total registered</div>
                  </div>
                          ...
                          <tr key={i} className="group hover:bg-white/[0.02] transition-colors">
                            ...
                            <td className="p-4">
                              <span className={`px-2 py-1 rounded border text-[10px] uppercase font-bold tracking-wider ${
                                u.role === 'founder' ? 'bg-indigo-500/10 border-indigo-500/20 text-indigo-400' :
                                u.role === 'recruiter' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' :
                            ...
                            <td className="p-4 text-slate-400 text-xs flex items-center gap-1">
                              <Clock className="w-3 h-3" />
                              {new Date(u.createdAt).toLocaleDateString()}
                            </td>
                            <td className="p-4 pr-0 text-right text-xs font-mono text-slate-600">
              ...
              {activeTab === 'reports' && (
                <div className="p-6 rounded-2xl bg-white/[0.02] border border-white/5 backdrop-blur-md">
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="text-xs text-slate-500">{dbReports.length} simulations globally</div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {dbReports.map((report, i) => (
                      <div key={i} className="p-4 rounded-xl bg-black/20 border border-white/5 hover:border-white/10 transition-colors group">
                        <div className="flex justify-between items-start mb-3">
                          <div>
                            <div className="font-semibold text-white">{report.candidateName}</div>
                            <div className="text-xs text-slate-400">{report.role}</div>
                          </div>
                        ...
                        <Link href={`/report/${report.sessionId}`} className="flex items-center justify-between w-full p-2 rounded bg-white/5 hover:bg-white/10 text-xs font-medium text-slate-300 transition-colors">
                          View Deep Report

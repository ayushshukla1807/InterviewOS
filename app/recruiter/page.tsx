        ...
        <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="space-y-4 mb-6">
                  <div>
                    ...
                    <input type="text" value={inviteData.name} onChange={e => setInviteData(p => ({...p, name: e.target.value}))} className="w-full bg-zinc-900/50 border-none rounded-xl px-4 py-3 text-sm text-white focus:border-emerald-500 outline-none" placeholder="John Doe" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-medium text-zinc-400  tracking-tight mb-2">Candidate Email</label>
                    <input type="email" value={inviteData.email} onChange={e => setInviteData(p => ({...p, email: e.target.value}))} className="w-full bg-zinc-900/50 border-none rounded-xl px-4 py-3 text-sm text-white focus:border-emerald-500 outline-none" placeholder="john@example.com" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-medium text-zinc-400  tracking-tight mb-2">Target Role</label>
                    <input type="text" value={inviteData.role} onChange={e => setInviteData(p => ({...p, role: e.target.value}))} className="w-full bg-zinc-900/50 border-none rounded-xl px-4 py-3 text-sm text-white focus:border-emerald-500 outline-none" placeholder="Software Engineer" />
                  </div>
                </div>
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {liveSessions.length === 0 ? (
                  <div className="col-span-full py-32 text-center rounded-[40px] border border-emerald-500/10 bg-[var(--theme-bg)]/40 backdrop-blur-3xl shadow-sm flex flex-col items-center justify-center relative overflow-hidden group">
                    ...
                    <div className="relative mb-8 z-10">
                      ...
                      <div className="w-24 h-24 bg-[#0A0A0A] rounded-full flex items-center justify-center border-2 border-emerald-500/40 shadow-sm relative z-10">
          ...
          {activeTab === 'candidates' && (
            ...
            >
              ...
              {isLoadingDb ? (
                <div className="py-24 flex justify-center"><div className="w-10 h-10 border-4 border-emerald-500 border-t-transparent rounded-full animate-spin shadow-sm"></div></div>
              ) : dbReports.length === 0 ? (
            ...
            >
              ...
              <div className="bg-zinc-950 border border-zinc-800 p-6 rounded-2xl mb-8 flex flex-col gap-4 relative overflow-hidden">
                ...
                <div className="flex gap-4 flex-col sm:flex-row">
                  <input type="text" value={jdInput} onChange={e => setJdInput(e.target.value)} placeholder="e.g. Senior Rust Backend Dev focusing on latency and WebSockets..." className="flex-1 bg-black/40 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-white focus:border-emerald-500 outline-none" />
                  <button onClick={() => {
